...
### 仍待定（开工前/对应阶段定）
- ~~薄弱"足够"的阈值~~ **已实现（默认值，2026-05-30 复核）**：`backend/services/learning_map.py` 硬编码 `MIN_PERSONAL_ATTEMPTS=5 / MIN_PERSONAL_MISTAKES=3 / MIN_PERSONAL_WEAKNESSES=2`，三者全满足 `default_entry=weakness`，否则默认 `kp`（即 P0-4，已落地+测试）。**遗留**：此为工程占位值、未经数据校准（当前真实数据 attempts=3/mistakes=2 尚不满足），待数据积累后再回看是否调整。
- ~~边聚合：物化 vs 实时算~~ **已定（2026-05-30）：实时算**——题型↔技巧/易错点边（`pattern_skills/pattern_pitfalls`）不落表，查询/渲染时由 `question_*` 边即时聚合，避免过早把数据结构复杂化；待实时计算体验不可接受时再优化为物化。注：LLM 题目级标签（`question_skills/question_pitfalls`，含出处/置信度/人审）仍照常落库，本决策只针对题型层的卷积结果。
...
### Finding 2：完成判据修订
- "完成"以**产品结果（面板内容覆盖率）**为准，非"函数能跑/测试 ==1"。验收脚本应断言：（按实时聚合结果）`pattern_skills` 覆盖的题型占比 ≥ 阈值、且每题型边数落在合理区间（不空、不爆）。
