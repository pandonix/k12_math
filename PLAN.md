...
POST /api/graph/patterns              # 手动维护题型（API 保留；前端入口暂时屏蔽，见下）
POST /api/graph/edges                 # 手动/LLM 校正图谱边
```

> **状态（2026-05-30）**：图谱页左侧「新增题型」表单已**暂时屏蔽**（`renderGraphView` 不再渲染该表单）。原因：手动维护题型的入口语义与图谱建模尚未想清楚（题型 vs 技巧/易错点的边界、手动新建题型如何与导入数据去重/合并）。后端 `POST /api/graph/patterns` 与 `submitPatternForm` 处理器均保留，待设计明确后重新接线恢复入口。
...
| 图谱 | `views/graph.js` | 基础学习图谱局部邻接查看：知识点 ⇄ 题型 ⇄ 技巧/易错点 ⇄ 题目。v1 不做全量知识图谱总览，避免边过多导致不可读；全量概览作为最低优先级可选演进（见 §13）。**左侧「新增题型」表单已暂时屏蔽（2026-05-30），待手动维护题型的设计明确后再恢复（见 §7.3）** |
